...
def run(fold, df, transforms_train, transforms_val, mel_idx):
    ...
    for epoch in range(1, int(config["n_epochs"]) + 1): 
        ...
        val_loss, acc, auc ,f1 = val_epoch(model, valid_loader, mel_idx)

        content = time.ctime() + ' ' + f'Fold {fold}, Epoch {epoch}, lr: {optimizer.param_groups[0]["lr"]:.7f}, train loss: {train_loss:.5f}, valid loss: {(val_loss):.5f}, acc: {(acc):.4f}, auc: {(auc):.6f}, f1:{(f1):.6f}.'
        print(content)  
...
#             auc_max = auc
        if f1 > f1_max:
            print('f1_max ({:.6f} --> {:.6f}). Saving model ...'.format(f1_max, f1))
            torch.save(model.state_dict(), model_file)
            f1_max = f1
